...
layui.use(['table', 'layer', 'element'], function () {
  ...
  table.render({
    elem: '#tableData',
    id: "tableReload",
    url: ApiService.SystemApi.APIService + '/Api/ProductType/GetData',
    headers: {
...
function deleteInfo(obj, gid) {
  layer.confirm('真的删除行么', function (index) {
    ...
    ajaxGet({
      url: ApiService.SystemApi.APIService + "/Api/ProductType/DeleteModel",
      data: {
        gid: gid
      },
...
function OpenDetail(title, type) {
  layer.open({
    ...
    yes: function (indexs, layero) {
      ...
      if (IsNullOrEmpty(model.TypeName)) {
        layer.msg("请填写名称");
        return false;
      }
      ajaxPost({
        url: ApiService.SystemApi.APIService + "/Api/ProductType/SaveModel",
        data: model,
        success: function (res) {
          layer.msg(res.message);
          if (res.code != 1) {

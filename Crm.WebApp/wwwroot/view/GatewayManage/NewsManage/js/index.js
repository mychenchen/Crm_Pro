...
layui.use(['table', 'layer'], function () {
    ...
    table.render({
        ...
        , cols: [[
            ...
            , { field: 'InformationSource', title: '信息来源' }
            , { field: 'CoverUrl', title: '封面图片', templet: '#imgShow' }
    ...
    table.on('tool(tableData)', function (obj) {
        ...
        if (obj.event === 'del') {
            layer.confirm('真的删除行么', function (index) {
                ...
                $.ajax({
                    url: ApiService.SystemApi.APIService + "/Api/HotNews/DeleteModel",
                    type: "get",
                    headers: { "ToKenStr": localStorage.Token },
                    data: { gid: data.Id },
...
function OpenDetail() {
    layer.open({
        ...
        yes: function (indexs, layero) {
            ...
            if (IsNullOrEmpty(model.ParentGid) || IsNullOrEmpty(model.ChildrenGid)) {
                layer.msg("请选择菜单");
                return false;
            }
            ...
            if (IsNullOrEmpty(model.CoverUrl)) {
                layer.msg("请上传图片");
                return false;
            }

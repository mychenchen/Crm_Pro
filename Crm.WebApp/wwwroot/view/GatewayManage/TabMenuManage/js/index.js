...
layui.use(['table', 'layer', 'element'], function () {
    ...
    table.render({
        elem: '#tableData'
        , id: "tableReload"
        , url: ApiService.SystemApi.APIService + '/Api/TabMenu/GetData'
...
function addRowChild(trIndex, tr, table, pid) {
    ...
    var _tablechild = table.render({
        elem: '#' + childTableName
        , url: ApiService.SystemApi.APIService + '/Api/TabMenu/GetDataByPid'
        , page: false //关闭分页
...
function deleteInfo(obj, gid) {
    layer.confirm('真的删除行么', function (index) {
        ...
        ajaxGet({
            url: ApiService.SystemApi.APIService + "/Api/TabMenu/DeleteModel",
            data: { gid: gid },
...
function OpenDetail(title, type) {
    layer.open({
        ...
        yes: function (indexs, layero) {
            ...
            ajaxPost({
                url: ApiService.SystemApi.APIService + "/Api/TabMenu/SaveModel",
                data: model,
                success: function (res) {
                    layer.msg(res.message);
                    if (res.code != 1) {
